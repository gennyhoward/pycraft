...
class Model(object):
    ...
    def _initialize(self):
        ...
        """
        return
        n = WORLD_SIZE  # 1/2 width and height of world
        s = 1  # step size
...
def MakeWorld(m, w):
    ...
    for x in range(-n, n + 1, s):
        for z in range(-n, n + 1, s):
            # create a layer stone an grass everywhere.
            m.add_block((x, y - 2, z), GRASS, immediate=True)
            m.add_block((x, y - 3, z), STONE, immediate=True)
            if x in (-n, n) or z in (-n, n):
                # create outer walls.
                for dy in range(-2, 3):
                    m.add_block((x, y + dy, z), STONE, immediate=True)
        ...
        for y in range(c, c + h):
            for x in range(a - s, a + s + 1):
                for z in range(b - s, b + s + 1):
                    ...
                    if (x - 0) ** 2 + (z - 0) ** 2 < 5 ** 2:
                        continue
                    m.add_block((x, y, z), t, immediate=True)
            s -= d  # decrement side lenth so hills taper off
...
def MakeWorld1(m):
    ...
    #for x in range(-n, n + 1, s):
    #    for z in range(-n, n + 1, s):
    #        m.add_block((x, y-10, z), GRASS, immediate=False)

    for w in range(-20,20):
        for x in range(-w, w + 1, s):
